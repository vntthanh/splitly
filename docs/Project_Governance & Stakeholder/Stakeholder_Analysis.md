...
Splitly is a proposed shared-expense web application for Vietnamese users. The MVP is planned to support authentication, groups and members, manual entry, equal/by-person/by-item splitting, Gemini 3.1 Flash Lite receipt drafts, item assignment, bill-level payment status, VietQR assistance, payment confirmation, reminders, and essential notifications.
